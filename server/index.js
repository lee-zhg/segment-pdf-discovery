...
const DEFAULT_NAME = 'pdf-segment';
const DEFAULT_PATH = './data/pdf-segment/';
var discoveryDocs = [];
const fs = require('fs');
const path = require('path');
var arrayOfFiles = fs.readdirSync(DEFAULT_PATH);
arrayOfFiles.forEach(function(file) {
  discoveryDocs.push(path.join(DEFAULT_PATH, file));
});
...
const discoverySetupParams = {
  ...
  config_name: 'config-pdf-seg'   // instead of 'Default Configuration',
};
  ...
  discoverySetup.setupDiscovery(discoverySetupParams, (err, data) => {
    if (err) {
      ...
      console.log('Begin loading ' + discoveryDocs.length +
        ' PDF file(s) into discovery. Please be patient as this can take several minutes.');
